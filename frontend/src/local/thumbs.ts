...
export function placeholderUrl(label: string): string {
  // Deliberately still. This is the tile for a decode that has *finished and
  // failed*, so it must not animate: a shimmer promises work in progress, and
  // on a format the browser will never open that promise is never kept. The
  // skeletons in the grid, filmstrip and loupe cover the "not yet" case; this
  // one covers "not ever", and the two should not look alike.
